(() => {
  ...
  const boot = () => {
    ...
    const ensurePanel = () => {
      ...
      const panel = document.createElement('aside'); panel.className='cooldown-panel';
      panel.innerHTML='<div class="cooldown-title">战斗时间 / CD</div><div class="cooldown-grid"><div id="cdS1">技能1 READY</div><div id="cdS2">技能2 READY</div><div id="cdSub">替身 READY</div><div id="cdScroll">密卷 READY</div></div>';
      wrap.appendChild(panel);
    };
    const refreshUi = () => {
      ensurePanel(); const f=window.A; if(!f)return;
      const set=(id,label,ms)=>{const el=document.getElementById(id);if(!el)return;el.textContent=`${label} ${format(ms)}`;el.classList.toggle('ready',ms<=0);el.classList.toggle('cooling',ms>0);};
      const p1={s1:remaining('p1','s1'),s2:remaining('p1','s2'),r:remaining('p1','r'),c:remaining('p1','c')};
      set('cdS1','技能1',p1.s1); set('cdS2','技能2',p1.s2); set('cdSub','替身',p1.r); set('cdScroll','密卷',p1.c);
      for(const [id,a] of [['skill1','s1'],['skill2','s2'],['skillSub','r'],['skillScroll','c']]){const el=document.getElementById(id);if(!el)continue;const ms=p1[a];el.classList.toggle('on-cd',ms>0);el.dataset.cooldown=ms>0?(ms/1000).toFixed(1):'READY';}
      f._cdSnapshot={skill1:p1.s1,skill2:p1.s2,substitute:p1.r,scroll:p1.c};
    };
    window.BIBLE_FIGHTER_COOLDOWN_READY = true;
    window.BIBLE_FIGHTER_COOLDOWN_API = { snapshot: () => ({version:state.version,now:now(),p1:{s1:remaining('p1','s2')?remaining('p1','s1'):0,s2:remaining('p1','s2'),r:remaining('p1','r'),c:remaining('p1','c')}}) };
    const loop = () => { refreshUi(); requestAnimationFrame(loop); };
    loop();
    return true;
  };
  const tryBoot=()=>boot();
  if(!tryBoot()){let tries=0;const timer=setInterval(()=>{tries++;if(tryBoot()||tries>=120)clearInterval(timer);},50);}
})();
