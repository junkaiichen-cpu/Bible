...
const required=['playtest.html','game7.js','game46.js','game47.js','electron/main.cjs','electron/preload-runtime.cjs','data/update.json','data/update-manifest.js'];
...
const main=readFileSync('electron/main.cjs','utf8');if(!main.includes('preload-runtime.cjs'))fail('Electron is not using 1.5.0 preload');
const preload=readFileSync('electron/preload-runtime.cjs','utf8');for(const m of ['1.5.0','game46.js','game47.js'])if(!preload.includes(m))fail(`preload marker missing: ${m}`);
...
for(const f of ['game7.js','game46.js','game47.js','electron/main.cjs','electron/preload-runtime.cjs']){const r=spawnSync(process.execPath,['--check',f],{stdio:'inherit'});if(r.status!==0)process.exit(r.status??1)}
