...
for (const marker of ['BIBLE_FIGHTER_STABILITY_READY','BIBLE_FIGHTER_TRIGGER_HITSTOP','BIBLE_FIGHTER_CLEAR_INPUT','bounds','separation']) if (!readFileSync('game32.js','utf8').includes(marker)) { console.error(`Build validation failed: stability marker missing: ${marker}`); process.exit(1); }
...
console.log('Build validation passed: combat runtime + HUD + identity + codex + briefing + frame rules + David art/impact + local 2P + 2D stages + production fighters + readability + stability + match lifecycle + canonical input reset present.');
