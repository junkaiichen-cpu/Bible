...
function writeFatalLog(kind, error) { try { const message = error instanceof Error ? `${error.stack || error.message}` : String(error); const logPath = path.join(app.getPath('userData'), 'bible-fighter-runtime.log'); fs.appendFileSync(logPath, `[${new Date().toISOString()}] ${kind}\n${message}\n`, 'utf8'); } catch {} }
  ...
  win.webContents.once('did-finish-load', () => {
    ...
    setTimeout(async () => {
      try {
        ...
        const stability = await win.webContents.executeJavaScript(`(() => { A.x=-500; B.x=1400; window.BIBLE_FIGHTER_STABILITY_READY; document.dispatchEvent(new KeyboardEvent('blur')); return {bounds:window.BIBLE_FIGHTER_STABILITY?.bounds||[],inputsCleared:!Object.values(window.BIBLE_FIGHTER_MULTIPLAYER?.input?.p1||{}).some(Boolean) && !Object.values(window.BIBLE_FIGHTER_MULTIPLAYER?.input?.p2||{}).some(Boolean), marker:!!window.BIBLE_FIGHTER_STABILITY_READY}; })()`, true);
        console.log(`[smoke] stability ${JSON.stringify(stability)}`);
        if (!stability.marker || stability.bounds.length!==2 || !stability.inputsCleared) { console.error(`Stability probe failed: ${JSON.stringify(stability)}`); smokeFailed=true; app.exit(1); return; }
