...
function writeSmokeTrace(stage, payload) {
  if (!smoke) return;
  try {
    smokeLogPath ||= path.join(app.getPath('userData'), 'bible-fighter-smoke.log');
    const body = typeof payload === 'string' ? payload : JSON.stringify(payload);
    fs.appendFileSync(smokeLogPath, `[${new Date().toISOString()}] ${stage} ${body}\n`, 'utf8');
  } catch {}
}

function failSmoke(stage, payload) {
  writeSmokeTrace(stage, payload);
  smokeFailed = true;
  app.exit(1);
}

process.on('uncaughtException', (error) => { writeFatalLog('uncaughtException', error); writeSmokeTrace('uncaughtException', error?.stack || error); if (smoke) { smokeFailed = true; app.exit(1); } });
process.on('unhandledRejection', (reason) => { writeFatalLog('unhandledRejection', reason); writeSmokeTrace('unhandledRejection', reason?.stack || reason); if (smoke) { smokeFailed = true; app.exit(1); } });
...
function createWindow() {
  ...
  win.webContents.on('did-fail-load', (_e, code, desc) => { if (!smoke) return; writeSmokeTrace('renderer-load-failed', {code, desc}); smokeFailed=true; app.exit(1); });
  win.webContents.on('render-process-gone', (_e, details) => { if (!smoke) return; writeSmokeTrace('renderer-gone', details); smokeFailed=true; app.exit(1); });
  win.webContents.on('console-message', (_e, level, message) => { if (!smoke) return; writeSmokeTrace('console', {level, message}); console.log(`[renderer:${level}] ${message}`); if (level >= 3) { smokeFailed=true; app.exit(1); } });
  win.webContents.once('did-finish-load', () => {
    if (!smoke) return;
    writeSmokeTrace('load-complete', { readyState:'complete' });
    setTimeout(async () => {
      try {
        ...
        writeSmokeTrace('selection', selected);
        console.log(`[smoke] selection ${JSON.stringify(selected)}`);
        if (!selected?.ok || selected.p1Label!=='大卫' || selected.p2Label!=='摩西' || selected.startDisabled) return failSmoke('selection-failed', selected);
        ...
        writeSmokeTrace('battle-start', started);
        ...
        if (!battleStarted) return failSmoke('battle-start-failed', started);

        const combatUi = await win.webContents.executeJavaScript(`(() => ({missionText:document.querySelector('#missionObjective')?.textContent||'',skill1:document.querySelector('#skill1 .skill-name')?.textContent||'',skill2:document.querySelector('#skill2 .skill-name')?.textContent||'',mapName:document.querySelector('#mapName')?.textContent||'',p1Identity:document.querySelector('#identityP1Name')?.textContent||'',p2Identity:document.querySelector('#identityP2Name')?.textContent||'',codexReady:!!window.BIBLE_FIGHTER_CODEX_READY,briefingReady:!!window.BIBLE_FIGHTER_BRIEFING_READY,framesReady:!!window.BIBLE_FIGHTER_COMBAT_FRAMES_READY,rulesReady:!!window.BIBLE_FIGHTER_FRAME_RULES_READY,davidArtReady:!!window.BIBLE_FIGHTER_DAVID_ART_READY,davidImpactReady:!!window.BIBLE_FIGHTER_DAVID_IMPACT_READY,multiplayerReady:!!window.BIBLE_FIGHTER_MULTIPLAYER_READY,stageReady:!!window.BIBLE_FIGHTER_STAGE_READY,stageCurrent:window.BIBLE_FIGHTER_STAGE?.current?.()||'',coreCharactersReady:!!window.BIBLE_FIGHTER_CORE_CHARACTERS_READY,coreRoster:window.BIBLE_FIGHTER_CORE_CHARACTERS?.roster||[],readabilityReady:!!window.BIBLE_FIGHTER_READABILITY_READY,stabilityReady:!!window.BIBLE_FIGHTER_STABILITY_READY,matchReady:!!window.BIBLE_FIGHTER_MATCH_READY}))()`, true);
        writeSmokeTrace('combat-ui', combatUi);
        console.log(`[smoke] combat-ui ${JSON.stringify(combatUi)}`);
        if (!combatUi.missionText || !combatUi.skill1 || !combatUi.skill2 || !combatUi.mapName || combatUi.p1Identity!=='大卫' || combatUi.p2Identity!=='摩西' || !combatUi.codexReady || !combatUi.briefingReady || !combatUi.framesReady || !combatUi.rulesReady || !combatUi.davidArtReady || !combatUi.davidImpactReady || !combatUi.multiplayerReady || !combatUi.stageReady || !combatUi.coreCharactersReady || !combatUi.coreRoster.includes('david') || !combatUi.coreRoster.includes('moses') || !combatUi.readabilityReady || !combatUi.stabilityReady || !combatUi.matchReady) return failSmoke('combat-ui-failed', combatUi);

        const moved = await win.webContents.executeJavaScript(`(() => new Promise(resolve => { const before = window.BIBLE_FIGHTER_MULTIPLAYER.snapshot(); document.dispatchEvent(new KeyboardEvent('keydown',{key:'d',bubbles:true})); document.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowLeft',bubbles:true})); setTimeout(()=>{ document.dispatchEvent(new KeyboardEvent('keyup',{key:'d',bubbles:true})); document.dispatchEvent(new KeyboardEvent('keyup',{key:'ArrowLeft',bubbles:true})); resolve({before,after:window.BIBLE_FIGHTER_MULTIPLAYER.snapshot()}); },220); }))()`, true);
        writeSmokeTrace('movement', moved);
        console.log(`[smoke] movement ${JSON.stringify(moved)}`);
        if (!(moved?.after?.p1?.x > moved?.before?.p1?.x) || !(moved?.after?.p2?.x < moved?.before?.p2?.x)) return failSmoke('movement-failed', moved);

        const stability = await win.webContents.executeJavaScript(`(() => { A.x=-500; B.x=1400; const ok=!!window.BIBLE_FIGHTER_STABILITY_READY; window.dispatchEvent(new Event('blur')); return {bounds:window.BIBLE_FIGHTER_STABILITY?.bounds||[],inputsCleared:!Object.values(window.BIBLE_FIGHTER_MULTIPLAYER?.input?.p1||{}).some(Boolean) && !Object.values(window.BIBLE_FIGHTER_MULTIPLAYER?.input?.p2||{}).some(Boolean),marker:ok}; })()`, true);
        writeSmokeTrace('stability', stability);
        console.log(`[smoke] stability ${JSON.stringify(stability)}`);
        if (!stability.marker || stability.bounds.length!==2 || !stability.inputsCleared) return failSmoke('stability-failed', stability);
        ...
        writeSmokeTrace('damage', damage);
        console.log(`[smoke] damage ${JSON.stringify(damage)}`);
        if (!(damage?.after < damage?.before)) return failSmoke('damage-failed', damage);

        const ko = await win.webContents.executeJavaScript(`new Promise(resolve => { B.hp=1; A.x=B.x-54; act('p1','a'); setTimeout(()=>resolve({p1:A.hp,p2:B.hp,over:!!S.over,resultHidden:document.querySelector('#result')?.classList.contains('hidden'),score:[...S.score],round:S.r}),(160)) })`, true);
        writeSmokeTrace('ko', ko);
        console.log(`[smoke] ko ${JSON.stringify(ko)}`);
        if (!(ko?.over || ko?.p2 <= 0 || ko?.resultHidden === false)) return failSmoke('ko-failed', ko);

        writeSmokeTrace('passed', { multiplayer: window.BIBLE_FIGHTER_MULTIPLAYER.snapshot(), match: window.BIBLE_FIGHTER_MATCH_SYNC?.snapshot?.() || null });
        console.log(`[smoke] multiplayer-final ${JSON.stringify(window.BIBLE_FIGHTER_MULTIPLAYER.snapshot())}`);
      } catch (error) {
        writeSmokeTrace('runtime-probe-exception', error?.stack || String(error));
