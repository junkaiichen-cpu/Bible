(() => {
  ...
  const boot = () => {
    ...
    const state=window.BIBLE_FIGHTER_COMBAT_CORE=window.BIBLE_FIGHTER_COMBAT_CORE||{version:'1.4.6',ready:true,actions:0,moves:0,jumps:0,guards:0,reversals:0,hits:0,airHits:0,lockOn:true,lastAction:''};
    ...
    window.hit=(attacker,defender,damage,stun,knock,label)=>{
      if(defender&&guarding(defender)){state.guards++;defender.st=Math.min(defender.st||0,4);defender.vx=(defender.vx||0)*0.35;defender.armor=Math.max(defender.armor||0,6);window.txt?.(defender.x+21,defender.y-14,'格挡','guard');window.burst?.(defender.x+21,defender.y+28,8,'#d8c9a6',2.4);return;}
    ...
    window.update=(f,dt)=>{originalUpdate(f,dt);if(!f||!window.S.run)return;const e=enemy(f);if(e&&state.lockOn&&f.st<=0&&f.lock<=0&&Math.abs((e.x||0)-(f.x||0))<210){f.f=((e.x||0)>f.x)?1:-1;f.face=f.f;}f._coreGuard=guarding(f);f._combatRange=e?Math.abs((e.x||0)-(f.x||0)):Infinity;};
